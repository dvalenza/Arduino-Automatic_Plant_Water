...
Servo myservo;
const int buttonPin = 2; //button to open
int buttonState = 0;   
const int buttonPin2 = 3; //door sensor
int buttonState2 = 0;  
void setup() {                
...
void wait(int time){
  for(int x=time; x>=0; x--){
    lcd.setCursor(0,1); //Start at character 1 on line2 
    lcd.print(x);
    delay(1000);
  }
  lcd.print("          ");
...
void loop()
{
  ...
  buttonState2 = digitalRead(buttonPin2);
    delay(20);
  if (buttonState == HIGH) { //Open door button pressed
    int x=1;
    while(x){//wait for release of button
    buttonState = digitalRead(buttonPin);
    delay(10);
    if (buttonState == LOW) { //button released
    myservo.write(20); //open door
    Serial.write("0");
    while(x){ //wait for door to open OR button to be pressed again
    delay(10);
      buttonState2 = digitalRead(buttonPin2);
      buttonState = digitalRead(buttonPin);
      delay(10);
      ...
      if(buttonState == HIGH) {//yellow button pressed again, hold door unlocked untill button press
        while(x){//wait for release
        delay(10);
          buttonState = digitalRead(buttonPin);
          delay(10);
          if (buttonState == LOW) {//button released
            while(x){//wait for button to be pressed a third time to lock door
          Serial.write("4");
          delay(10);
          buttonState = digitalRead(buttonPin);
          delay(10);
          if(buttonState == HIGH) {
            while(x){
              buttonState = digitalRead(buttonPin);
              if(buttonState == LOW) { //on button release, lock door
              x=0;
              myservo.write(120);
              break;
          }}}
      ...
      if(buttonState2 == LOW) {  //door opened
        while(x){//wait to close lock
          Serial.write("2");
          buttonState2 = digitalRead(buttonPin2);
          if(buttonState2 == HIGH) { //door closed
              //buttonState2 = digitalRead(buttonPin2);  ??
              Serial.write("3");
              x = 0;
              break;
